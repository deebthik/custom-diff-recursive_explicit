...
echo "+ -> Only in $dir1"
echo "- -> Only in $dir2"
echo "_ -> Common files/folders that differ, containing only the (readable) differences"
...
do
	...
	then
		...
		#stores the diff of text files as usual, but when it's a non-readable file like a zip, apk, bin, etc., it stores the diff of the hexdumps of both files
		if [ $(file "$currentpath" | grep "ASCII" | wc -c) -gt 0 ] && [ $(file "$currentpath2" | grep "ASCII" | wc -c) -gt 0 ]
		then
			diff "$currentpath" "$currentpath2" > "$destdir/$name"
		else
			diff -y <(xxd "$currentpath") <(xxd "$currentpath2") > "$destdir/$name"".hex"
		fi
...
echo "To view the colored diff tree contents again, just do a simple cat or less -r -> 'cat $diffdeets/difftreecolored.txt'"
echo
